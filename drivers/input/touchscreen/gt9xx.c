...
#if ROTATE90DEGREE		//逆时针 x,y值交换  Y值取 Y-720取绝对值
	realX=y;
	realY=x;
	realY=(s32)abs(realY-X_WIDTH);
#else					//顺时针90 x,y值交换  X值取 X-1280取绝对值
	realX=y;
	realY=x;
	realX=(s32)abs(realX-Y_WIDTH);
#endif*/
...
realY=y;
    ...
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, realX);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, realY);
 ...
 */
    ...
    {
        ts->abs_x_max = X_WIDTH;
        ts->abs_y_max = Y_WIDTH;
        ts->int_trigger_type = 1; //falling
    }
